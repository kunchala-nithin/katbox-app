...
export default function ChefOrderReviewScreen() {
  ...
  const [isMapAddressModalVisible, setIsMapAddressModalVisible] = useState(false);
  const [editingAddressId, setEditingAddressId] = useState<string | null>(null);
  const [mapInitialCoords, setMapInitialCoords] = useState({
    latitude: 17.3850,
    longitude: 78.4867,
  });
  const [mapInitialPinnedAddress, setMapInitialPinnedAddress] = useState("");
  const [mapInitialHouseDetail, setMapInitialHouseDetail] = useState("");
  const [mapInitialCustomTagTitle, setMapInitialCustomTagTitle] = useState("");
  ...
  const getDeliveryPrice = (delivery: string) => {
    ...
    if (delivery === "Doorstep") return 503.77;
    if (delivery === "Doorstep + Service") return 1025.73;
    return 0;
  };
  ...
  // ────────────────────────────────────────────────
  // Occasion options — Puja image replaced with a
  // reliable, visible Pexels URL showing puja/diya items.
  // ────────────────────────────────────────────────
  const occasions = [
    { id: 1, name: "Birthday Party", image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQyt7jcl4ZbrWd5CnT-uRVTHhhvo1JLVp3iZx8aE7JcYA&s=10" },
    { id: 2, name: "House Party", image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRKyk4GnCfEwn2eARupiXnS5gFBP5MUSOX2oKEWaFT6DQ&s=10" },
    { id: 3, name: "Family Get-Together", image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRPZfOjJmb1zBuDC70SgIycq1_fFCOkvfMGUGqmyzK55g&s=10" },
    { id: 4, name: "Kitty Party", image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRUIDZbD0tNMZ0WjuPgCnjY83Ldw1PXj40rKhx8LAN29Q&s=10" },
    { id: 5, name: "Corporate Events", image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSsebVsEh0WeuFmDTuPokmj55-QiD58_-DTUFJ2RJQA4w&s=10" },
    // ✅ Puja — visible image showing diya / puja thali (Pexels, very reliable)
    { id: 6, name: "Puja", image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTbx1tHAfhuByVLiZq13i0rRXQNhkot2eMXeprh8Y-K1A&s=10" },
    { id: 7, name: "House Warming", image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR5L1pGGIZXnGT72Jj_AqKrGyxPQhZax2cnq5vGXq6Uzw&s=10" },
    { id: 8, name: "Farm House Party", image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcScAZT6osWDOEN1SvWfaYwY4qnpEC35j7n7iq6ZbwrIHA&s=10" },
    { id: 9, name: "Workshops", image: "https://images.unsplash.com/photo-1531482615713-2afd69097998?w=600&auto=format&fit=crop&q=80" },
    { id: 10, name: "Other", image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTeJt1cHfnW_VPocnF-SNkt_o7lKT0oZlBbEGPZn50lYw&s=10" },
  ];
  ...
  const handleOpenAddNewAddress = () => {
    ...
    setAddressModal(false);
    setIsMapAddressModalVisible(true);
  };
  ...
  const handleOpenEditAddress = (item: SavedAddress) => {
    ...
    setMapInitialAddressTag(item.tag || "Home");
    setAddressModal(false);
    setIsMapAddressModalVisible(true);
  };
  ...
  const handleMapClose = () => {
    setEditingAddressId(null);
    setIsMapAddressModalVisible(false);
  };
  ...
  const isAddressDuplicate = (
    ...
    const normalizedHouse = houseDetails.trim().toLowerCase();

    return list.find((item) => {
      if (editingAddressId && item.id === editingAddressId) return false;

      const sameAddress =
        String(item.fullAddress || "").trim().toLowerCase() === normalizedAddress &&
        String(item.houseDetails || "").trim().toLowerCase() === normalizedHouse;

      const sameCoordinates =
        Math.abs(Number(item.latitude || 0) - latitude) < 0.0001 &&
        Math.abs(Number(item.longitude || 0) - longitude) < 0.0001;
  ...
  const handleMapConfirm = async (payload: AddressMapConfirmPayload) => {
    ...
    const trimmedAddress = String(pinnedAddress || "").trim();
    const trimmedHouse = String(houseDetail || "").trim();
    const tagInput = String(customTagTitle || "").trim();
    ...
    const title =
      addressTag === "Other" && tagInput.length > 0
        ? tagInput
        : addressTag;

    try {
      let updatedAddresses: SavedAddress[] = [...savedAddresses];
      ...
      if (editingAddressId) {
        ...
        if (duplicate) {
          ...
          updatedAddresses = [newAddressItem, ...savedAddresses];
      ...
      applyActiveAddressToUI(newActive);
      setSelectedAddressId(newActive.id || "");
      ...
      await setSavedAddressesForUser(
        ...
        updatedAddresses,
        newActive,
        formatAddressDisplay(newActive)
      );
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      >
            ...
            <View style={[styles.modalBox, { maxHeight: height * 0.74 }]}> 
              ...
              <View style={styles.modalSheetHeaderRow}>
                <View>
                  ...
                  <Text style={{ fontSize: 12, color: theme.darkGray, marginTop: 2, fontWeight: "500" }}>
              ...
              <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: height * 0.5 }}>
                ...
                {isLoadingAddress ? (
                  ...
                  savedAddresses.map((item) => {
                    ...
                    return (
                      <View key={item.id} style={styles.addressCardItemRow}>
                        <TouchableOpacity
                        ...
                        <View style={styles.addressCardActions}>
                          <TouchableOpacity
                            style={styles.addressSmallAction}
                            activeOpacity={0.7}
                  ...
                  <View style={styles.emptySavedBox}>
                    ...
                    <Text style={styles.emptySavedSubtitle}>
                      Add a new address by pinning your exact location on the map.
                    </Text>
                  </View>
                )}

                <TouchableOpacity style={styles.addAddressBtn} onPress={handleOpenAddNewAddress} activeOpacity={0.8}>
                  <Ionicons name="map-outline" size={18} color={theme.primaryPurple} />
                  <View style={{ flex: 1, marginLeft: 8 }}>
                    <Text style={styles.addAddressText}>Add New Address</Text>
                    <Text style={{ fontSize: 10.5, color: theme.darkGray, marginTop: 1 }}>
        ...
        {/* ADDRESS MAP MODAL FROM COMPONENTS */}
        <AddressMapModal
          visible={isMapAddressModalVisible}
